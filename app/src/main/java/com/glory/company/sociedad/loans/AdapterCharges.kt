...
class AdapterCharges(private val trans: ArrayList<Trans>, private val context: Context) : RecyclerView.Adapter<AdapterCharges.ViewHolder>() {
    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): AdapterCharges.ViewHolder {
        val v = LayoutInflater.from(p0.context).inflate(R.layout.recycler_view_trans, p0, false)
        return ViewHolder(v)
    }

    override fun onBindViewHolder(p0: AdapterCharges.ViewHolder, p1: Int) {
                                                ...
                                                docRef.delete().addOnCompleteListener { task ->
                                                    if (task.isSuccessful) {
                                                        db.collection("databases").document(database).get().addOnCompleteListener {
                                                            ...
                                                            db.collection("databases").document(database).update("cash",(box-total)).addOnSuccessListener {
